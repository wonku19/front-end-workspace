...
// h4 중 마지막 요소의 글자 크기를 2rem으로 변경
$('h4').last().css("font-size", "2rem");
// h4 중 5번째 요소의 텍스트를 '테스트-5'로 변경
$('h4:eq(4)').text('테스트-5');
// 클래스 선택자 : test를 선택해서 홀수 인덱스 요소만 배경색상 : royalblue, 글자색 : white 로 변경
$('h4.test').odd().css({
    'background-color': 'royalblue',
    'color': 'white'
});
// 클래스 선택자 : test를 선택해서 짝수 인덱스 요소만 배경색상 : hotpink, 글자색 : white 로 변경
$('h4.test').even().css({
    'background-color': 'hotpink',
    'color': 'white'
});
// h4 중 클래스 선택자 test가 아닌 요소를 선택해서 배경색상 : chocolate, 글자색 : white 로 변경
$('h4').not('.test').css({
...
console.log($('h4').slice('.test'));
console.log($('h4').slice('.test1'));

// 2. Ancestors & Decendants 메소드
...
//.wrap인 요소의 자식의 자식의 자식 요소 중에 li태그의 style4를 적용
...
// h2 요소의 형제 요소들에 style3 적용하기
$('.wrap2 h2').siblings().css(style3);
// h2 요소의 형제 요소 중 p 요소에 style2 적용하기
$('.wrap2 h2').siblings('p').css(style2);
// h2 요소의 다음 형제 요소에 style1 적용하기
$('.wrap2 h2').next().css(style1);
// h2 요소의 다음에 오는 모든 형제 요소에 배경색 : pink 적용하기
