...
- `$('선택자').is('선택자')` : 선택된 요소들 중 특정 요소가 있는지 확인 (ture, false를 리턴)
...
2. `$('생성구문').insertAfter('선택자')` : 특정 요소를 선택자로 지정한 요소의 형제 요소로 뒷부분에 추가
2. `$('생성구문').insertBefore('선택자')` : 특정 요소를 선택자로 지정한 요소의 형제 요소로 앞부분에 추가
...
- bind() 메소드는 3.0버턴부터 사용하지 않기 때문에 on() 메소드를 사용할 것을 권장
...
- 마우스 이벤트 : **click**, dblclic, **mouseenter**, **mouseleave**, mousedown, mouseup, mouseover, mouseout, mousemove
...
## hover()
- mouseenter 이벤트와 mouseleave 이벤트에 동시에 연결
```js
$('선택자').hover(
    function() {}, // mouseenter
    function() {}  // mouseleave
